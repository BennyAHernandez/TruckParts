...
function registraCliente(array $datos, $con){
    ...
    $sql = $con->prepare("INSERT INTO cliente (nombre, apellido_pat, apellido_mat, correo, telefono, curp, estatus, fecha_alta) 
    VALUES (?,?,?,?,?,?,1,now())");
...
function usuarioExiste($usuario, $con){
    ...
    $sql = $con->prepare("SELECT id_usuario FROM usuario WHERE usuario LIKE ? LIMIT 1");
    $sql->execute([$usuario]);
...
function emailExiste($correo, $con){
    ...
    $sql = $con->prepare("SELECT id_cliente FROM cliente WHERE correo LIKE ? LIMIT 1");
    $sql->execute([$correo]);
    if($sql->fetchColumn() > 0){
        return true;
    }
...
function validaToken($id, $token, $con){
    ...
    $sql = $con->prepare("SELECT id_usuario FROM usuario WHERE id_usuario = ? AND token LIKE ? LIMIT 1");
...
function activarUsuario($id, $con){
    $sql = $con->prepare("UPDATE usuario SET activo = 1, token = '' WHERE id_usuario = ?");
...
function login($usuario, $password, $con, $proceso){
    $sql = $con->prepare("SELECT id_usuario, usuario, password, id_cliente FROM usuario WHERE usuario LIKE ? LIMIT 1");
    $sql->execute([$usuario]);
...
function esActivo($usuario, $con){
    $sql = $con->prepare("SELECT activo FROM usuario WHERE usuario LIKE ? LIMIT 1");
    $sql->execute([$usuario]);
